    ...
    .array(
      z.object({
        ...
        description: z
          .string()
          .nullish()
          .describe("Key responsibilities or achievements"),
      }),
    )
...
export async function POST(req: Request) {
  try {
    ...
    console.log(
      `📄 [Resume Extract] Extracted ${result.object.skills?.length ?? 0} skills, ${result.object.experience?.length ?? 0} experiences`,
    );
