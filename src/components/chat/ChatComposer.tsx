...
import { ArrowUp, X, ChevronDown, Loader2, Check, Lock, AlertTriangle, MessageSquare, Compass } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 pb-6">
      ...
      <div className="relative">
        {/* Tabs - Protruding from top-left */}
        {onModeChange && (
          <div className="absolute -top-8 left-0 flex z-10">
            <button
              type="button"
              onClick={() => onModeChange("jay")}
              className={clsx(
                "flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-all rounded-t-lg border border-b-0",
                mode === "jay"
                  ? "bg-white text-blue-700 border-slate-200"
                  : "bg-slate-100 text-slate-400 border-transparent hover:text-slate-600"
              )}
              style={mode === "jay" ? { boxShadow: "0 -4px 12px rgba(0, 109, 255, 0.15)" } : undefined}
            >
            ...
            <button
              ...
              className={clsx(
                "flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-all rounded-t-lg border border-b-0 -ml-px",
                mode === "navigator"
                  ? "bg-white text-violet-700 border-violet-200"
                  : "bg-slate-100 text-slate-400 border-transparent hover:text-slate-600"
              )}
              style={mode === "navigator" ? { boxShadow: "0 -4px 12px rgba(139, 92, 246, 0.15)" } : undefined}
            >
        ...
        <form
          ...
          className={clsx(
            ...
            onModeChange && "mt-8", // Space for tabs
            isNavigatorMode
              ? "border-violet-200"
              : "border-slate-200",
